...
public class Settings extends Constants{
	 ...
	 */
	public static final String APP_FILE = "propertyFiles/device.properties";
	...
	public void beforeClassInBase(String appType, String device,String browser, String environment) throws MessagingException {
		...
		try {
			...
			else  if (appType.equals("emulator")) {
	...
	public static void androidEmulatorSettings() throws IOException {



		System.out.println("Setting up the andriod studio capabilities..");
		...
		androidCapabilities.setCapability(MobileCapabilityType.PLATFORM_VERSION, "13");

		androidCapabilities.setCapability(MobileCapabilityType.DEVICE_NAME,"Android Emulator");

		androidCapabilities.setCapability(MobileCapabilityType.NEW_COMMAND_TIMEOUT,280);

		androidCapabilities.setCapability(MobileCapabilityType.APP, userDirectory+"/app/android/v0.1.27.0.1-QA.apk");
		...
		try {
			...
			System.out.println("Appium server is successfully configured and started");
	...
	public static void iOSSettings() throws InterruptedException, IOException {
		System.out.println("Stopping the previous Appium Server");
		//		stopServer();
		System.out.println("Starting a new Appium server");
		...
		try {
			driver = new IOSDriver<>(new URL("http://127.0.0.1:4723/wd/hub"),iOSCapabilities);//:4723 is default port appium -p 4725
			System.out.println("Appium server is successfully configured and started");
			System.out.println("Running the test");
		}catch(Exception e) {
			Log.addMessage("Appium Server not started. Plesae refer error log for more details");
	...
	public static void androidSettings() throws IOException {
		...
		//	androidCapabilities.setCapability("appPackage", "com.android.settings");
		//	androidCapabilities.setCapability("appActivity", "Settings");
		//androidCapabilities.setCapability("appActivity", "Settings");
		try {
			//			driver = new AndroidDriver<>(new URL("http://127.0.0.1:4723/wd/hub"), androidCapabilities);
			driver = new AndroidDriver<>(new URL("http://127.0.0.1:4723/wd/hub"), androidCapabilities);
	...
	public static String getPageURL() {
		...
		else if(environment.equals("beta")) {//added on 11 june 21
			System.out.println("inside beta");
			url = PropertyUtility.getProperty("BetaUrl");
			System.out.println("iBetaUrl="+url);
			env = "beta";
			testdata="beta";
			subject = PropertyUtility.getProperty("BetaUrl");
		}
		else {
			url = PropertyUtility.getProperty("QAUrl");
			env="QA";
